...
Đặt `dev-knowledge-base` **ngang hàng** các project (cùng thư mục tổng). Rồi:
```bash
cd ~/Work/mywork      # thư mục tổng
kb bootstrap          # nối MỌI project một lần (--all để gồm cả thư mục chưa git)
```
...
nào (idempotent). Nối một project lẻ: `kb link`.
...
| Lệnh | Làm gì |
|---|---|
| `kb bootstrap [--all]` | Nối tất cả project vào KB (ở thư mục tổng) |
...
> Slash command (`/kb-scan`, `/kb-drift`, `/kb-onboard`) **tự cài** khi `kb link`/`bootstrap` — không cần bước riêng.
...
```
dev-knowledge-base/
├── kb                   # ⭐ CLI một cổng — kb bootstrap | link | scan | check | hook | help
...
└── scripts/             # bootstrap-all, link-kb, connect-project, install-hooks, install-claude-hook, kb-drift-check, detect-stack
```
