...
| Lệnh | Làm gì |
|---|---|
| `kb init [--all]` | Nối tất cả project vào KB (hoặc `/kb-init` trong Claude Code). Alias: `kb bootstrap` |
...
| `kb drift` | In hướng dẫn drift check **sâu (AI)** — không tự chạy, chỉ nhắc gõ `/kb-drift` trong Claude Code |
...
| `kb path` | In đường dẫn tuyệt đối tới KB |
| `kb help` | Trợ giúp |

> `kb check` (mechanical, tự chạy) khác `kb drift` (chỉ in hướng dẫn để chạy AI qua `/kb-drift`). Đừng nhầm hai lệnh.
...
└── scripts/             # init-all, link-kb, install-hooks, install-claude-hook, kb-drift-check, detect-stack (connect-project: deprecated → link-kb)
```
